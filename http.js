 ...
 *   4. No env vars needed (key comes per-request from the Authorization header).
 *
...
/**
 * Health check endpoint — useful for Perplexity to verify the connector is live.
 */
...
/**
 * Handle a single MCP request/response cycle.
 * We create a fresh McpServer per request (stateless — Jules holds all session state).
 */
async function handleMcp(request) {
  const apiKey = extractApiKey(request);
  if (!apiKey) {
    return new Response(
      JSON.stringify({ error: "Missing API key. Set Authorization: Bearer <your Jules API key> in the connector settings." }),
  ...
  // Wire server → transport, then handle the request.
...
async function handleRequest(request) {
  ...
  if (url.pathname === "/mcp") {
    ...
    // Some clients send an OPTIONS preflight
