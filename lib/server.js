...
function sourceIdPathCandidates(sourceId) {
  ...
  // Strip any leading "sources/" prefix to get the raw id segment
  const rawId = normalized.replace(/^sources\//, "");
  const candidates = [
    `/sources/${rawId}`,
    `/sources/${encodeURIComponent(rawId)}`,
  ];
  // Also try with explicit "sources/" prefix if not already present
 ...
 *
 * In stdio mode: called from index.js with baseUrl from CLI args / env.
 * In HTTP mode:  called per-request from http.js with apiKey from Authorization header.
 */
export function createServer(apiKey, baseUrl) {
  ...
  async function listSourcesWithLocalFilter({ pageSize, pageToken, filter }) {
    ...
    let scannedPages = 0;
    const matches = [];
    // Track the last page token BEFORE the page that pushed us over the limit.
    // This gives callers a token that resumes from the correct boundary.
    ...
    // Return the token for the page we stopped at so callers can continue
    // from the same position without seeing duplicates.
    return {
      sources: matches,
