 ...
 *
 * AUDIT FIXES (v0.5.0):
 *   - Removed jules_wait_for_session (wrong architecture for 5min-1hr async jobs)
 *   - jules_get_session_output: removed fake 'files' field (not in Jules API spec)
 *   - jules_rename_session: honest description — no rename endpoint exists
 *   - jules_list_activities_filtered: paginate all pages when activityType filter set
 *   - jules_clone_session: copies automationMode from original session
 *   - jules_quick_session + jules_create_session: added automationMode param
 *   - jules_get_latest_activity: follows nextPageToken to truly last page
 *   - jules_build_session_prompt: requirePlanApproval defaults to false (aligned with rest of API)
 */
...
function extractSessionOutputs(session) {
  // NOTE: Jules API spec only defines 'pullRequest' in SessionOutput.
  // The 'files' field is not part of the v1alpha spec and will never be populated.
  const outputs = { pullRequests: [], raw: session?.outputs || [] };
...
const TOOL_REFERENCE = {
  ...
  jules_create_session: {
    ...
    params: [
      ...
      { name: "automationMode", type: "string", required: false, description: "Jules automation mode. Use AUTO_CREATE_PR to have Jules open a PR automatically.", example: "AUTO_CREATE_PR" },
  ...
  jules_list_activities_filtered: {
    purpose: "List session activities with optional type filtering. Paginates all pages when activityType is set.",
    params: [
      { name: "sessionId", type: "string", required: true, description: "Session identifier.", example: "12345678901234567890" },
      { name: "pageSize", type: "integer", required: false, description: "Number of activities per page (1-100).", default: 30, example: 30 },
      { name: "pageToken", type: "string", required: false, description: "Pagination token from previous response (only used when activityType is not set).", example: "NEXT_PAGE_TOKEN" },
      { name: "activityType", type: "string", required: false, description: "Filter by activity type (e.g., ACTIVITY_COMPLETED). When set, all pages are fetched.", example: "ACTIVITY_COMPLETED" },
    ],
  },
  jules_get_session_output: {
    purpose: "Extract pull request outputs from a completed session. Note: Jules API only exposes pullRequest outputs — files are not available via API.",
    params: [
      { name: "sessionId", type: "string", required: true, description: "Session identifier.", example: "12345678901234567890" },
      { name: "outputType", type: "string", required: false, description: 'Output type: "pullRequest" or "all". Note: "files" is not supported by the Jules API.', default: "all", example: "pullRequest" },
    ],
  },
  ...
  jules_build_session_prompt: {
    ...
    params: [
      ...
      { name: "requirePlanApproval", type: "boolean", required: false, description: "Whether to require plan approval before execution (default: false).", default: false, example: true },
    ],
  },
  jules_clone_session: {
    purpose: "Clone an existing session: copy its prompt, source, branch, and automationMode into a new session.",
    params: [
  ...
  jules_get_latest_activity: {
    purpose: "Get only the most recent activity from a session (last agent message or status update). Follows pagination to get the truly last activity.",
    params: [{ name: "sessionId", type: "string", required: true, description: "Session identifier.", example: "12345678901234567890" }],
  },
  ...
  jules_rename_session: {
    purpose: "Send an informational note to a session with a new title. The Jules API has no native rename endpoint — the session title in Jules UI will not change.",
    params: [
      { name: "sessionId", type: "string", required: true, description: "Session identifier.", example: "12345678901234567890" },
      { name: "newTitle", type: "string", required: true, description: "New title to record as a note.", example: "Fix auth bug - v2" },
    ],
  },
};
...
export function createServer(apiKey, baseUrl) {
  ...
  function buildSkillPayload({ toolName, compact, includeExamples }) {
    ...
    return {
      server: { name: "jules-mcp-server", version: "0.5.0" },
      focus: "jules-tooling",
      ...
      usageHints: [
        ...
        "Use automationMode: AUTO_CREATE_PR on jules_create_session or jules_quick_session to have Jules open a PR without manual intervention.",
        "Jules takes 5 minutes to 1 hour per session. Use jules_get_session_state to poll between AI turns — do not use jules_wait_for_session.",
      ],
      tools: compact
  ...
  function buildSessionTemplate(templateName, customParams = {}) {
    ...
    // requirePlanApproval defaults to false — aligned with create_session default
    const requirePlanApproval = typeof customParams.requirePlanApproval === "boolean" ? customParams.requirePlanApproval : false;
    return { title: customParams.title || template.title, prompt: customParams.prompt || template.prompt, requirePlanApproval };
  }

  // ---- McpServer -----------------------------------------------------------

  const server = new McpServer({ name: "jules-mcp-server", version: "0.5.0" });
  ...
  server.registerTool("jules_create_session",
    {
      ...
      inputSchema: z.object({
        ...
        automationMode: z.string().optional().describe("Jules automation mode. Use AUTO_CREATE_PR to have Jules open a PR automatically."),
  ...
  server.registerTool("jules_list_activities_filtered",
    {
      description: "List activities for a session with optional type filtering. When activityType is set, all pages are fetched before filtering.",
      inputSchema: z.object({
        sessionId: z.string().min(1),
        pageSize: z.number().int().min(1).max(100).optional(),
        pageToken: z.string().optional(),
        activityType: z.string().optional().describe("Filter by activity type: ACTIVITY_QUEUED, ACTIVITY_RUNNING, ACTIVITY_COMPLETED, etc. When set, all pages are fetched."),
      })
    },
    async ({ sessionId, pageSize, pageToken, activityType }) => {
      const sid = requireNonEmpty(sessionId, "sessionId");

      // When no filter: single-page passthrough (preserves existing pagination behaviour)
      if (!activityType) {
        const result = await julesRequest({ path: `/sessions/${encodeURIComponent(sid)}/activities`, query: { pageSize, pageToken } });
        return textResult({ ...result, filterApplied: { type: null } });
      }

      // When filtering: paginate ALL pages then filter client-side
      const allActivities = [];
      let nextToken = undefined;
      const maxPages = 20;
      let pages = 0;
      do {
        const page = await julesRequest({ path: `/sessions/${encodeURIComponent(sid)}/activities`, query: { pageSize: pageSize || 100, pageToken: nextToken } });
        const batch = Array.isArray(page.activities) ? page.activities : [];
        allActivities.push(...batch);
        nextToken = optionalTrimmed(page.nextPageToken);
        pages += 1;
      } while (nextToken && pages < maxPages);

      const filtered = allActivities.filter((a) => a?.type === activityType);
      return textResult({
        activities: filtered,
        filterApplied: { type: activityType, matchCount: filtered.length, scannedTotal: allActivities.length, pagesScanned: pages },
      });
    }
  );
  ...
  server.registerTool("jules_get_session_output",
    {
      description: "Extract structured outputs from a completed session. Only pull request outputs are available — the Jules API does not expose file-level outputs. Use jules_list_pr_outputs to scan multiple sessions.",
      inputSchema: z.object({
        sessionId: z.string().min(1),
        outputType: z.enum(["pullRequest", "all"]).optional().default("all"),
      })
    },
    async ({ sessionId, outputType }) => {
      ...
      return textResult({
        sessionId,
        state: session?.state,
        pullRequests: extracted.pullRequests,
        note: "Jules API only exposes pullRequest outputs. File-level outputs are not available via the API.",
      });
    }
  );
  ...
  server.registerTool("jules_health_check",
    ...
    async () => {
      try {
        ...
        return textResult({ status: "healthy", apiReachable: true, version: "0.5.0", timestamp: new Date().toISOString() });
      } catch (error) {
        return textResult({ status: "unhealthy", apiReachable: false, version: "0.5.0", error: error?.message || String(error), timestamp: new Date().toISOString() });
      }
    }
  );
  ...
  server.registerTool("jules_clone_session",
    {
      description: "Clone an existing session: copy its prompt, source, starting branch, and automationMode into a brand new session. Useful for retrying failed sessions.",
      inputSchema: z.object({
        sessionId: z.string().min(1),
    ...
    async ({ sessionId, newTitle, newPrompt, requirePlanApproval }) => {
      ...
      if (typeof requirePlanApproval === "boolean") body.requirePlanApproval = requirePlanApproval;
      // Carry automationMode forward from the original session
      const originalAutomationMode = optionalTrimmed(original?.automationMode);
      if (originalAutomationMode) body.automationMode = originalAutomationMode;
  ...
  server.registerTool("jules_quick_session",
    {
      description: "One-shot convenience tool: resolves a source by substring filter, applies a template, and creates a new session immediately.",
      inputSchema: z.object({
        template: z.enum(["add_tests", "fix_bug", "refactor", "review", "add_docs", "add_types", "security_audit", "add_ci", "upgrade_deps", "add_readme"]),
    ...
    async ({ template, sourceFilter, startingBranch, customPrompt, automationMode, requirePlanApproval }) => {
  ...
  server.registerTool("jules_get_latest_activity",
    {
      description: "Fetch only the most recent activity from a session. Follows pagination to get the truly last activity even for long-running sessions.",
      inputSchema: z.object({ sessionId: z.string().min(1) })
    },
    async ({ sessionId }) => {
      const sid = requireNonEmpty(sessionId, "sessionId");
      ...
      // Paginate all the way to the last page — Jules API returns oldest-first
      do {
        lastPage = await julesRequest({ path: `/sessions/${encodeURIComponent(sid)}/activities`, query: { pageSize: 100, pageToken: nextToken } });
        const batch = Array.isArray(lastPage.activities) ? lastPage.activities : [];
        totalScanned += batch.length;
        nextToken = optionalTrimmed(lastPage.nextPageToken);
        pages += 1;
      } while (nextToken && pages < maxPages);

      const activities = Array.isArray(lastPage?.activities) ? lastPage.activities : [];
      const latest = activities.length > 0 ? activities[activities.length - 1] : null;
      return textResult({
        sessionId: sid,
        totalActivities: totalScanned,
        pagesScanned: pages,
  ...
  server.registerTool("jules_list_pr_outputs",
    {
      description: "Scan recent sessions and return a list of those that produced pull requests.",
      inputSchema: z.object({
  ...
  server.registerTool("jules_rename_session",
    {
      description: "Send an informational note to a session recording a new title. The Jules API has no native rename endpoint — the session title in the Jules UI will NOT change. Use this only to leave a note for tracking purposes.",
      inputSchema: z.object({
        sessionId: z.string().min(1),
    ...
    async ({ sessionId, newTitle }) => {
      ...
      } catch (_) { /* non-fatal — session may not accept messages in current state */ }
      const session = await julesRequest({ path: `/sessions/${encodeURIComponent(sid)}` });
      return textResult({
        note: "The Jules API does not support native session renaming. A note was sent to the session only. The title shown reflects the original Jules title.",
